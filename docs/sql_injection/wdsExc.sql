...
-- EX. 4:
SELECT DISTINCT b.name FROM albums a
JOIN bands b ON a.band_id = b.id;

-- EX. 5:
SELECT DISTINCT b.name FROM albums a
FULL OUTER JOIN bands b ON a.band_id = b.id
WHERE a.id IS NULL;
...
GROUP BY s.album_id, a.name, a.release_year
...
-- EX. 8:
INSERT INTO bands(id, name) VALUES (8, 'Oscar');
INSERT INTO albums(id, name, release_year, band_id) VALUES (19, 'Stefan IV', 2024, 8);
...
SELECT b.name AS "Band", COUNT(s.id) AS "Number of Songs" FROM bands b
JOIN albums a ON a.band_id = b.id
JOIN songs s ON s.album_id = a.id
GROUP BY b.name;